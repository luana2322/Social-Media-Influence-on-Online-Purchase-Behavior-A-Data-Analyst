        ...
        <div className="mt-6 space-y-2">
          ...
          {shapData.slice(0, 5).map((feature) => (
            <div key={feature.feature} className="flex items-start gap-2 p-2 rounded-lg bg-accent/30">
              ...
              />
              <p className="text-sm">
                <span className="font-medium">{feature.feature}</span> contributes
                <span className="font-semibold text-primary"> +{(feature.importance * 100).toFixed(0)}%</span> to purchase probability
              </p>
            </div>
          ))}
        </div>
